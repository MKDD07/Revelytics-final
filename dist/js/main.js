...
document.addEventListener("DOMContentLoaded", function () {
	...
	if ($('.reveal-text').length) {
		...
		if (textheading.length > 0 && typeof SplitText !== "undefined") {
			...
			textheading.each(function (index, el) {
				try {
					...
					gsap.set(el.split.chars, { opacity: 1, x: 0 });
					gsap.fromTo(el.split.chars,
						{ opacity: 0.3, x: -7 },
						{
							scrollTrigger: {
								trigger: el,
    ...
    window.ms_magnetic = ms_magnetic;
    $(function () {
        ms_magnetic();
    });
	...
	function initJumpAnim() {
		try {
			...
			$items.forEach(item => {
				if (item.classList.contains('jump-anim-init')) return;
				item.classList.add('jump-anim-init');
	...
	window.initJumpAnim = initJumpAnim;
	document.addEventListener("DOMContentLoaded", initJumpAnim);
	$(function () {
		initJumpAnim();
	});
	...
	if (typeof SplitText !== "undefined") {
		gsap.utils.toArray(".tp-text-perspective").forEach(splitTextLine => {
			try {
				const delay_value = parseFloat(splitTextLine.getAttribute("data-delay") || 0.2);
				...
				gsap.set(splitTextLine, { perspective: 400, opacity: 1, visibility: 'visible' });
				itemSplitted.split({ type: "lines" });

				tl.fromTo(itemSplitted.lines,
					{ opacity: 0, rotationX: -50, y: 20 },
					{
						duration: 0.8,
						delay: delay_value,
						opacity: 1,
				...
				$(splitTextLine).css({ opacity: 1, visibility: 'visible' });
			}
		});
	}
	...
	if ($(".tp_fade_anim").length > 0) {
		gsap.utils.toArray(".tp_fade_anim").forEach((item) => {
			let tp_fade_offset = parseFloat(item.getAttribute("data-fade-offset") || 30),
				tp_duration_value = parseFloat(item.getAttribute("data-duration") || 0.75),
				tp_fade_direction = item.getAttribute("data-fade-from") || "bottom",
				tp_onscroll_value = item.getAttribute("data-on-scroll") || 1,
				tp_delay_value = parseFloat(item.getAttribute("data-delay") || 0.1),
				tp_ease_value = item.getAttribute("data-ease") || "power2.out";

			gsap.set(item, { opacity: 1, visibility: "visible" });

			let startX = (tp_fade_direction == "left" ? -tp_fade_offset : (tp_fade_direction == "right" ? tp_fade_offset : 0));
			let startY = (tp_fade_direction == "top" ? -tp_fade_offset : (tp_fade_direction == "bottom" ? tp_fade_offset : 0));
	...
	///////////////////////////////////////////////////
	// 41. carachter Animation
	if ($('.tp-char-animation').length > 0 && typeof SplitText !== "undefined") {
		...
		char_come.forEach(splitTextLine => {
			try {
				...
				gsap.set(splitTextLine, { perspective: 300, opacity: 1, visibility: 'visible' });
				itemSplitted.split({ type: "chars, words" });
				tl.fromTo(itemSplitted.chars,
					{ opacity: 0, x: 40 },
					{
						duration: 0.8,
						delay: 0.2,
