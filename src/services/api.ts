 ...
 * Routes through the Cloudflare Worker proxy at /api/pexels which queries live Pexels search.
 */
export async function fetchPexelsPhoto(query: string): Promise<string | null> {
  ...
  try {
    ...
    console.warn(`fetchPexelsPhoto request failed for query "${query}":`, err);
    return null;
  }
}
